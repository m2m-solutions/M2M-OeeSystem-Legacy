...
namespace M2M.DataCenter.Reports
{
    ...
    [Description("Npb")]
    [DisplayName("01")]
    public partial class NpbOee : Telerik.Reporting.Report
    {
        #region Properties
        ...
        {
            #region Page header
            textBox26.Value = ResourceStrings.GetString("Report.NpbOee.Header");
            textBox24.Value = ResourceStrings.GetString("Npb.Module#:");
            textBox17.Value = ResourceStrings.GetString("Npb.Unit#:");
            #endregion

            #region Overview
            textBox29.Value = ResourceStrings.GetString("ScheduledProductionTime#:");
            tbProductionTime.Format = String.Format("{0} {1}", "{0:N1}", ResourceStrings.GetString("#-Hours.Abbr"));
            textBox3.Value = ResourceStrings.GetString("AvailabilityBasedOnActualStopsNpb#:");
            textBox32.Value = ResourceStrings.GetString("ItemsHandled#:");
            textBox2.Value = ResourceStrings.GetString("Oee.Npb#:");
            textBox1.Value = ResourceStrings.GetString("AvailabilityWithAbbr#:");
            textBox36.Value = ResourceStrings.GetString("Performance.Npb#:");
            textBox37.Value = ResourceStrings.GetString("QualityWithAbbr#:");
            #endregion

            #region Oee charts
            chartPeriodicOee.Series[0].Name = ResourceStrings.GetString("Oee.Npb");
            ...
            textBox39.Value = ResourceStrings.GetString("Oee.Npb");
            textBox30.Value = ResourceStrings.GetString("Availability.Abbr");
            textBox85.Value = ResourceStrings.GetString("AvailabilityBasedOnActualStops.Abbr");
            ...
            textBox48.Value = ResourceStrings.GetString("Performance.Npb.Abbr");
            ...
            textBox8.Value = ResourceStrings.GetString("TimeBetweenStops");
            #endregion
        ...
        private void Machine_NeedDataSource(object sender, EventArgs e)
        {
            ...
            DateTime startDate = (DateTime)report.Parameters["RangeStart"].Value;
            DateTime endDate = (DateTime)report.Parameters["RangeEnd"].Value;
        ...
        {
            (sender as Telerik.Reporting.Processing.Table).DataSource = m_AggregatedDataContainer.GetTopStopTimesNpbSpecial(0, false).OrderByDescending(s => s.ElapsedTime).OrderBy(s => s.Machine);
        }
        ...
        private void chartPeriodicOee_NeedDataSource(object sender, EventArgs e)
        {
            ...
            //double minValue = oeeData.Min(a => a.OeeNpb);
            //Telerik.Reporting.Processing.Chart chart = sender as Telerik.Reporting.Processing.Chart;
            //Telerik.Reporting.Chart chartDef = (Telerik.Reporting.Chart)chart.ItemDefinition;
            //if (minValue <= 0.30)
            //{
        ...
        public static string GetDocumentName(object division, object rangeStart, object rangeEnd)
        {
            DateTime start = (DateTime)rangeStart;
            DateTime end = (DateTime)rangeEnd;
            return String.Format(ResourceStrings.GetString("Report.NpbOee.DocumentName"), M2MDataCenter.GetDivision((string)division).DisplayName, start.ToString("yyyyMMdd-HH:mm"), end.ToString("yyyyMMdd-HH:mm"));
        }
