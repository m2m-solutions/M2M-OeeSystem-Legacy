...
namespace M2M.DataCenter.Reports
{
    ...
    [Description("Default")]
    [DisplayName("08")]
    public partial class DivisionProductionWeekly : Telerik.Reporting.Report
    {
        #region Properties
        ...
        public DivisionProductionWeekly()
        {
            /// <summary>
        ...
        {
            #region Page header
            tbHeader.Value = ResourceStrings.GetString("Report.DivisionProductionWeekly.Header");
            tbSubHeader.Value = ResourceStrings.GetString("#+Report.DivisionProductionWeekly.SubHeader");
            tbDivisionPrompt.Value = M2MDataCenter.GetAvailableModules().Contains("Npb") ? ResourceStrings.GetString("Npb.Unit#:") : ResourceStrings.GetString("Division#:");
        ...
        public static string GetDocumentName(object division, object year, object week)
        {
            return String.Format(ResourceStrings.GetString("Report.DivisionProductionWeekly.DocumentName"), M2MDataCenter.GetDivision(division.ToString()).DisplayName, year, week);
        }
        ...
        private void Report_NeedDataSource(object sender, EventArgs e)
        {
            ...
            m_ProductionData.Division = M2MDataCenter.GetDivision(divisionId).DisplayName;
            m_ProductionData.PeriodicData = new List<MachineOee>();
            
            for(DateTime d = startTime; d < endTime; d = d.AddDays(1))
            {
                m_ProductionData.PeriodicData.Add(new MachineOee(0, divisionId, null, -1, d, d.AddDays(1), null));
            }
        ...
        private void SetGraphDataSource(object sender, int index)
        {
            ...
            graph.DataSource = prodData.PeriodicData[index].GetDataForAllMachines();
        }
