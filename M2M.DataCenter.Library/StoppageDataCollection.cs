...
{
	[Serializable()]
	public partial class StoppageDataCollection : BusinessListBase<StoppageDataCollection, StoppageData>
	{
		#region Business Methods

		public StoppageData Add(string divisionId, string machineId, string articleNumber, int shift, bool systemError, int categoryId)
		{
			StoppageData item = StoppageData.NewStoppageDataChild();
		...
		public static StoppageDataCollection NewStoppageDataCollection()
		{
			return new StoppageDataCollection();
		}

		public static StoppageDataCollection GetStoppageDataCollection(string divisionId, string machineId, SmartDate startTime, SmartDate endTime)
		{
			return DataPortal.Fetch<StoppageDataCollection>(new Criteria(divisionId, machineId, startTime, endTime));
		}

		private StoppageDataCollection()
			...
			using (SqlConnection connection = new SqlConnection(Database.SystemConnectionString))
			{
				...
				using (SqlCommand command = connection.CreateCommand())
				{
					...
					using (SafeDataReader dataReader = new SafeDataReader(command.ExecuteReader()))
					{
						...
						{
							Add(StoppageData.GetStoppageDataChild(dataReader));
						}
					}
				}
			}
		...
		protected override void DataPortal_Update()
		{
			...
			foreach (StoppageData item in DeletedList)
				item.DeleteSelf();
			DeletedList.Clear();

			foreach (StoppageData item in this)
				if (item.IsNew)
					item.Insert();
				else
					item.Update();
