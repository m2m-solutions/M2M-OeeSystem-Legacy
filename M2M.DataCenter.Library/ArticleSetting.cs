...
{
	[Serializable()]
	public partial class ArticleSetting : BusinessBase<ArticleSetting>
	{
		...
		internal static ArticleSetting NewArticleSetting()
		{
			if (!CanAddObject())
		...
		internal static ArticleSetting NewArticleSetting(string machineId, string articleNumber, string name)
		{
			if (!CanAddObject())
				throw new System.Security.SecurityException("User not authorized to add MachineSetting objects");
			return new ArticleSetting(machineId, articleNumber, name);
		}

		internal static ArticleSetting GetArticleSetting(SafeDataReader dr)
		{
			if (!CanGetObject())
				throw new System.Security.SecurityException("User not authorized to view MachineSetting objects");
			return new ArticleSetting(dr);
		}

		protected ArticleSetting()
			: base()
		{
			MarkAsChild();
		}
		...
		protected ArticleSetting(string machineId, string articleNumber, string name)
			: this()
		{
		...
		protected ArticleSetting(SafeDataReader dr)
			: this()
		{
			Fetch(dr);
		}
		...
		internal void Insert()
		{
			...
			using (SqlCommand command = connection.CreateCommand())
			{
                ...
                command.CommandText = @"INSERT INTO ArticleSettings 
                                        (MachineId,ArticleNumber,Setting,Value,LastModified) 
                                        VALUES 
                                        (@MachineId,@ArticleNumber,@Setting,@Value,@LastModified)";
		...
		internal void Update()
		{
			...
			{
				command.CommandType = CommandType.Text;
                command.CommandText = @"UPDATE ArticleSettings" +
		...
		internal void DeleteSelf()
		{
			...
			{
				command.CommandType = CommandType.Text;
                command.CommandText = @"DELETE ArticleSettings" + 
	                                  @" WHERE " +
	                                  @"MachineId=@MachineId" +
                                      @" AND ArticleNumber=@ArticleNumber" +
	                                  @" AND Setting=@Setting";
