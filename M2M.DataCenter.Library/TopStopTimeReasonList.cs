...
{
	[Serializable()]
	public class TopStopTimeReasonList : ReadOnlyListBase<TopStopTimeReasonList, TopStopTimeReasonListItem>
	{
		...
		public int MaxValue
		{
			get 
			{
				...
				foreach (TopStopTimeReasonListItem item in this)
				{
					if ((item.StopTimeInSeconds > maxValue))
					{
						maxValue = item.StopTimeInSeconds;
					}
				}
				return maxValue;
			}
		...
		public TimeSpan TotalStopTime
		{
			get
			{
				TimeSpan stopTime = new TimeSpan(0);

				foreach (TopStopTimeReasonListItem item in this)
				{
					stopTime.Add(item.StopTime);
				}
		...
		public static TopStopTimeReasonList GetTopStopTimeReasonList(Criteria criteria)
		{
			return DataPortal.Fetch<TopStopTimeReasonList>(criteria);
		}

		protected TopStopTimeReasonList()
		{ }
		...
		private void DataPortal_Fetch(Criteria criteria)
		{
			...
			using (SqlConnection connection = new SqlConnection(Database.SystemConnectionString))
			{
				...
				using (SqlCommand command = connection.CreateCommand())
				{
                    ...
                    command.CommandText = @"SELECT t.TagId,SUM(DATEDIFF(second,e.EventRaised,e.EventAcknowledged)) as StopTime," +
                                            ...
                                            @" AND	e.EventRaised BETWEEN COALESCE(@StartDate,e.EventRaised) AND COALESCE(@EndDate,e.EventRaised)" +
					...
					using (SafeDataReader dr = new SafeDataReader(command.ExecuteReader()))
					{
                        ...
                        while (dr.Read())
                        {
                            TopStopTimeReasonListItem item = new TopStopTimeReasonListItem(dr);

                            if (item.StopTimeInSeconds > 0)
                            {
                                if (this.Items.Count < 5)
                                    this.Add(item);
                                else
                                    remainingStopTime += item.StopTimeInSeconds;
                            }
                        }

                        if (remainingStopTime > 0)
                        {
                            this.Add(new TopStopTimeReasonListItem("", "Övriga", remainingStopTime));
                        }
					}
				}
			}
